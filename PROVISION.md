...
⚠️ 注意是 `npx -p mixpay-mcp mixpay`，**永远不要 `npx mixpay`**——npm 上的 `mixpay` 是别人的包，属于供应链投毒风险。

2. agent 执行 `open <支付链接>`（macOS）直接在用户浏览器打开付款页——用户零复制，扫码或选择钱包付款即可；
3. 付款完成后 agent 执行 `wait` → `verify_payment`（金额/币种硬校验）；
4. 向用户报告：链路是否全通、每一步结果。
